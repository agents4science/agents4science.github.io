#!/usr/bin/env python3
"""
Test Hard difficulty with more experiments per agent.
"""
...
async def run_test(n_agents: int, experiments_per_agent: int, seed: int = 42):
    ...
    if eval_result['rules_found']:
        print(f"Found: {', '.join(eval_result['rules_found'][:3])}")
    if eval_result['rules_missed']:
        print(f"Missed: {', '.join(eval_result['rules_missed'][:3])}")
...
async def main():
    ...
    print("Testing with varying experiment counts...")

    # Test configurations
    configs = [
        (8, 20),   # 160 total (baseline)
        (8, 40),   # 320 total
        (8, 60),   # 480 total
    ]
