"""
AgentsAcademyDashboard - Rich dashboard for the Academy multi-agent pipeline.

This wraps the simple AgentsAcademy example with a full-screen Rich dashboard
...
# Add parent directory to path so we can import from AgentsAcademy
sys.path.insert(0, os.path.join(os.path.dirname(__file__), "..", "AgentsAcademy"))
...
AGENT_CLASSES = [ScoutAgent, PlannerAgent, OperatorAgent, AnalystAgent, ArchivistAgent]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run Academy multi-agent pipeline with Rich dashboard."
    )
    parser.add_argument(
        "--goals",
        "-g",
...
def build_status_table(goals: list[str], statuses: list[dict]) -> Table:
    ...
    table = Table(
        title="[bold]Academy Multi-Agent Pipeline[/bold]",
        expand=True,
        pad_edge=True,
    )
...
async def run_goal(
    ...
    """Run the five agents in sequence for a single goal using Academy Manager."""
    ...
    state = goal
    for agent_class, name in zip(AGENT_CLASSES, AGENTS):
        status_row[name] = "[yellow]Running"
        refresh_ui(name)

        # Launch agent via Manager and call its act method
...
async def run_headless(goals: list[str], statuses: list[dict]) -> None:
    ...
    async with await Manager.from_exchange_factory(
        ...
        for idx, goal in enumerate(goals):
            ...
            print("=" * 60)
            await run_goal(
                manager, idx, goal, statuses[idx], None, None, goals, statuses
            )


async def run_with_dashboard(goals: list[str], statuses: list[dict]) -> None:
    ...
    with Live(layout, refresh_per_second=4, console=console) as live:
        async with await Manager.from_exchange_factory(
            ...
            for idx, goal in enumerate(goals):
                await run_goal(
                    manager, idx, goal, statuses[idx], live, layout, goals, statuses
                )
...
async def main() -> None:
    ...
    if SHOW_UI:
        await run_with_dashboard(goals, statuses)
    else:
        await run_headless(goals, statuses)
