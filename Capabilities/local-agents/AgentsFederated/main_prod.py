...
class ANLComputeAgent(Agent):
    ...
    async def run_simulation(self, parameters: dict) -> dict:
        ...
        print(f"[ANL] Starting simulation with {parameters}")
        ...
        print(f"[ANL] Simulation complete: energy={result['result']['energy']}")
        return result
...
class ORNLDataAgent(Agent):
    ...
    async def query_database(self, query: str) -> dict:
        ...
        print(f"[ORNL] Querying database: {query}")
        ...
        print(f"[ORNL] Found {result['records_found']} records")
        return result
...
class LBNLAnalysisAgent(Agent):
    ...
    async def analyze_data(self, data: dict, method: str) -> dict:
        ...
        print(f"[LBNL] Analyzing data with method: {method}")
        ...
        print(f"[LBNL] Analysis complete: confidence={result['analysis_result']['confidence']}")
        return result
...
class FederatedCoordinator(Agent):
    ...
    async def register_agent(self, institution: str, agent: Handle) -> None:
        ...
        print(self._agents[institution])
...
async def main(task: str) -> None:
    ...
    anl_endpoint = 'abc' # specify globus_compute_endpoint running on ANL node
    ornl_endpoint = 'abc' # specify globus_compute_endpoint running on ORNL node
    lbnl_endpoint = 'abc' # specify globus_compute_endpoint running on LBNL node
    

    # Executors
    anl_executor = GCExecutor(anl_endpoint)
    ornl_executor = GCExecutor(ornl_endpoint)
    lbnl_executor = GCExecutor(lbnl_endpoint)
    ...
    async with await Manager.from_exchange_factory(
        ...
        manager.add_executor("anl_exec", anl_executor)
        manager.add_executor("ornl_exec", ornl_executor)
        manager.add_executor("lbnl_exec", lbnl_executor)

        # Launch institutional agents at its respective institution
        # Ensure that the remote machines have workers >= no. of agents on that machine
        anl = await manager.launch(ANLComputeAgent, executor="anl_exec")
        ornl = await manager.launch(ORNLDataAgent, executor="ornl_exec")
        lbnl = await manager.launch(LBNLAnalysisAgent, executor="lbnl_exec")

        logger.info("Launched all institutional agents")

        # Coordinator will be launched locally on the machine where main function is executed. 
        coordinator = await manager.launch(
            FederatedCoordinator,
        )

        logger.info('coordinator has been launched')
